...
# Host Licensing
$License = "0449P-65K82-38H4W-0LCK2-0M6MM"
...
Import-CSV vmhosts.csv | ForEach-Object {
  ...
  # Disable IPV6 System Wide
	...
	# Configure vSwitch0
		...
		Add-VirtualSwitchPhysicalNetworkAdapter -VirtualSwitch $vs0 -VMHostPhysicalNic (Get-VMHostNetworkAdapter -Physical -Name $vmnic[5]) -Confirm:$false
...
<#	# Removes "VM Network" from the vSwitch0 and Creates a new VM Network
	...
	# Configure vSwitch1
		...
		Add-VirtualSwitchPhysicalNetworkAdapter -VirtualSwitch $vs1 -VMHostPhysicalNic (Get-VMHostNetworkAdapter -Physical -Name $vmnic[1],$vmnic[3]) -Confirm:$false
		...
		Get-VirtualPortGroup -name $iSCSIAName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicUnused $vmnic[3]
		New-VMHostNetworkAdapter  -PortGroup ISCSI-B -VirtualSwitch $vs1 -IP $IscsiBiP -SubnetMask $IscsiSubnetMask -VMotionEnabled: $false
		Get-VirtualPortGroup -name $iSCSIBName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicActive $vmnic[3]
